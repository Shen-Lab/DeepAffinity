...
linear = rnn_cell_impl._linear  # pylint: disable=protected-access #MK commented
...
def embedding_rnn_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
    ...
    enc_outputs, encoder_state = rnn.static_rnn(encoder_cell, encoder_inputs, dtype=dtype) #MK
    ...
    if isinstance(feed_previous, bool):
     outputs, states = embedding_rnn_decoder(  #MK
          decoder_inputs,
          encoder_state,
          cell,
          num_decoder_symbols,
     ...
     return outputs, states,encoder_state,enc_outputs #MK
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        outputs, state = embedding_rnn_decoder(
            ...
            update_embedding_for_previous=False) #MK
        state_list = [state]
    ...
    state = state_list[0]
    if nest.is_sequence(encoder_state): #MK
      state = nest.pack_sequence_as(
          structure=encoder_state, flat_sequence=state_list) #MK
    return outputs_and_state[:outputs_len], state,encoder_state,enc_outputs   #MK 
...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    if isinstance(feed_previous, bool):
      outputs, states = embedding_attention_decoder( # MK change
          decoder_inputs,
          encoder_state,
          attention_states,
          cell,
          ...
          feed_previous=feed_previous,
          initial_state_attention=initial_state_attention)
      return outputs, states, encoder_state,encoder_outputs # MK change
    ...
    return outputs_and_state[:outputs_len], state, encoder_state,encoder_outputs #MK change
...
def model_with_buckets(encoder_inputs,
  ...
  outputs = []
  states = []  #MK change
  enc_outputs = [] #MK change
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(
          variable_scope.get_variable_scope(), reuse=True if j > 0 else None):
        bucket_outputs,_,bucket_states,bucket_enc_outputs= seq2seq(encoder_inputs[:bucket[0]], #MK change 
                                    decoder_inputs[:bucket[1]])
        outputs.append(bucket_outputs)
        states.append(bucket_states) #MK change
        enc_outputs.append(bucket_enc_outputs) #MK change
        if per_example_loss:
          losses.append(
              sequence_loss_by_example(
                  outputs[-1],
  ...
  return outputs, losses,states,enc_outputs # MK change
